...
def __render_timeseries(config: Settings, summary: dict) -> dict:
    varid = summary["varid"]
    ...
    table_tsfresh = []
    tsfresh_info = None
    if "tsfresh_features" in summary:
        for key, value in summary["tsfresh_features"].items():
            table_tsfresh.extend(
                [
                    {
                        "name": key,
                        "value": fmt_numeric(value, precision=config.report.precision),
                    },
                ]
            )

        tsfresh_table = Table(
            table_tsfresh, name="Tsfresh Features", style=config.html.style,
        )

        tsfresh_info = Container(
            [tsfresh_table],
            anchor_id=f"{varid}tsfresh_features",
            name="Other Statistics",
            sequence_type="grid",
        )

    if not table_tsfresh:
        ...
        template_variables["bottom"] = Container(
            [statistics, hist, fq, evs, acf_pacf, tsfresh_info],
            sequence_type="tabs",
            anchor_id=f"{varid}bottom",
        )
